...
pop_plot <- ggplot(data = county_socio_join) +
  geom_sf(aes(fill = total_pop, geometry = geometry), color = "white", size = 0.1) +
...
Is the poverty score a good indicator of water quality?
compare using NLS
```{r}
    ...
    tabPanel( ### start tab 4
      ...
      column(width = 3,
                    ...
                    radioButtons(
                      ...
                      choices = c('total_pop', 'ces', 'low_birth_weight', 'cardio_disease', 'education', 'poverty', 'unemplpoyment')
                    )
        ), ### end column
        column(width = 9,
                    h3('Map Here'),
...
### START tab 4, row 1
  soc_select <- reactive({ ### start soc_select
    soc_df <- county_socio_join %>% 
      filter(vector == input$factor_4_1)
    ...
    output$ces_plot <- renderPlot({ ### start ces_plot
       ggplot(data = soc_select) +
        geom_sf(aes(fill = vector, geometry = geometry), color = "white", size = 0.1) +
        scale_fill_gradientn(colors = c("lightgray", "orange","red")) +
        theme_void() +
